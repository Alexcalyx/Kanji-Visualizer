...
function ExampleWordListContent({ examples = [] }) {
    ...
    const handlePlayAudio = (audioData) => {
      ...
      const audioSrc = audioData?.mp3 || audioData?.ogg || audioData?.aac || audioData?.opus;
      if (audioSrc) {
    ...
    return (
        <>
            {examples && examples.length > 0 ? (
                ...
                <div>
                    <ul className="space-y-0"> {/* Remove vertical space between li elements */}
                        {examples.map((ex, index) => (
                            <li
                                key={ex.id}
                                // Added bottom border, increased padding, removed hover/bg/rounded
                                className={`py-2 ${index < examples.length - 1 ? 'border-b border-gray-200' : ''}`}
                            >
...
ExampleWordListContent.propTypes = {
...
export default ExampleWordListContent;
