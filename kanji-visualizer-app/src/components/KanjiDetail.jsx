...
const InfoItem = ({ label, children }) => (
    <div>
        ...
        <div className="text-base md:text-lg font-medium text-black">{children || 'N/A'}</div>
    </div>
);
...
function KanjiDetailReplicaFinalLayout() {
    ...
    if (detailsLoading) return ( <div className="flex flex-col items-center justify-center min-h-[50vh] text-gray-500"> {/* Loading... */} </div> );
    if (detailsError) return ( <div className="mt-10"> 
    <ErrorDisplay message={detailsError} context={`Kanji Details for ${character}`} /> Error loading details. </div> ); // Simplified error display placeholder
    if (!details || !details.character) return ( <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-xl p-10 text-gray-600 font-sans"> No valid details available for '{decodeURIComponent(character || '')}'. </motion.div> );
            ...
            */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-6 md:gap-y-8 mb-8 items-start">
                ...
                <div className="space-y-4 md:col-span-1 lg:col-span-1">
                    ...
                    <InfoItem label="On'yomi">{details.readings_on?.join('、 ')}</InfoItem>
                    <InfoItem label="Kun'yomi">{details.readings_kun?.join('、 ')}</InfoItem>
                    <div>
                ...
                <div className="space-y-4 md:col-span-1 lg:col-span-1">
                     ...
                     {details.hint && (
                         <div>
                             ...
                             <p className="text-base text-gray-700 leading-relaxed"> {/* Slightly larger hint text */}
            ...
            <div className="border-t pt-6 mt-8">
                <h3 className="text-base md:text-lg font-semibold text-gray-700 mb-3">Stroke Sequence</h3>
                {details.strokeImages && details.strokeImages.length > 0 ? (
                    // This responsive grid should adapt well already.
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(3rem,1fr))] gap-1 p-1 sm:p-2"> {/* Adjusted padding */}
