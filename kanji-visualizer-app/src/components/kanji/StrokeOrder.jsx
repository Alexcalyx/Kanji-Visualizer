...
function StrokeOrder({
  ...
  const isLoading = character && !strokeOrderUrl && !strokeOrderGif;
  const hasVideo = !!strokeOrderUrl;
  const hasPoster = !!strokeOrderGif; // Use poster even if video exists

  const displayContent = () => {
    if (!character) {
      // Don't show spinner if no character is selected yet
      return (
        <p className="text-sm text-center text-gray-400">No Kanji selected.</p>
      );
    }
    if (isLoading) {
      return (
        <div className="flex justify-center items-center h-full">
          <LottieSpinner size="md" />
        </div>
      );
    }
    if (hasVideo) {
      return (
        <video
          key={strokeOrderUrl} // Force re-render if URL changes
          controls
          poster={strokeOrderGif || ""} // Use SVG as poster
        ...
        >
          <source src={strokeOrderUrl} type="video/mp4" />
...
StrokeOrder.propTypes = {
  ...
  strokeOrderData: PropTypes.object,
  strokeOrderUrl: PropTypes.string,
  strokeOrderGif: PropTypes.string,
};
